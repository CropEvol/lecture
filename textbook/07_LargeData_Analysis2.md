...
<a name="section1"></a>
### Pandasについて
　Pandasは、図のようなデータセットの取り扱いを得意としたライブラリです。図では、一行に１個体分のデータで、各列にはその列名に関するデータが並んでいます。  このような構造のデータを、Pandasでは __データフレーム__ と呼んでいます。
...
　Pandasは高速処理されるように設計されているため、自身でfor文やif文を書いてデータ処理するよりも、Pandasを使った方がデータ処理速度は早いです。(*1)。また、上のような処理が一行のコードでできるため、書くコードの量も少なくて済みます。  
...
　この生物には赤色の個体と白色の個体がいます（このような違いのことを __多型__ といいます）。色の違う2個体のゲノムを調べたところ、ゲノム上に等間隔で分布する50個の遺伝子に違いが見られました。それら遺伝子について、赤色個体のアリル（対立遺伝子）を`R`、白色個体のアリルを`W`とすると、2個体は図のように表現できます。この50個の遺伝子のうち、1個が色に関わる遺伝子です。その遺伝子を見つけるのが今回の解析です。
...
　色遺伝子であれば、子孫集団の赤色個体はすべて`R`アリルであり、白色個体はすべて`W`アリルになっているはずです。色の違いに関与しない遺伝子はその限りではありません。
...
<a name="section3"></a>
### 演習:データ解析
　さきにPandasデータフレームの扱い方を勉強した後、データ解析をしていきます。
