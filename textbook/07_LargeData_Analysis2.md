...
<a name="section1"></a>
### Pandasについて
　Pandasは、図のようなテーブルの取り扱いを得意としたライブラリです。図では、一行に１個体分のデータで、各列にはその列名に関するデータが並んでいます。  このような構造のデータを、Pandasでは __データフレーム__ と呼んでいます。
...
　この生物には赤色の個体と白色の個体がいます（このような違いのことを __多型__ といいます）。色の違う2個体のゲノムを調べたところ、ゲノム上に等間隔で分布する50個の遺伝子に違いが見られました。それら遺伝子について、赤色個体のアリル（対立遺伝子）を`R`、白色個体のアリルを`W`とすると、2個体は下図左上のように表現できます。この50遺伝子のうち、どれか1つが色に関わっている遺伝子です。その遺伝子を見つけるのが今回の解析です。

　50遺伝子が異なる2個体だけではどれが色遺伝子なのか特定できないので、50遺伝子が様々な組合せになった個体群を用意する必要があります。そこで、赤色個体と白色個体を交配し、染色体の様々な領域に組換えが生じた子孫集団200個体を作成しました（下図左下）。  

　この子孫集団の表現型（赤色か白色か）と50遺伝子の遺伝子型（どちらの親由来のアリルを持っているか）を調べ、表にすると下図右のようになります。
　もし色遺伝子であれば、子孫集団の赤色個体はすべて`R`アリルであり、白色個体はすべて`W`アリルになっているはずです。色の違いに関与しない遺伝子はその限りではありません（つまり、赤色個体に`W`アリルが見られる遺伝子や、白色個体に`R`アリルが見られる遺伝子は、色に無関係な遺伝子です）。

<div style="margin-bottom: 5px;"><img src="../images/07/07_population.png" alt="dataframe"></div>

　そこで、50遺伝子の中から、条件に合う遺伝子を見つけるために、子孫集団を赤色個体グループと白色個体グループに分け、両グループで各遺伝子のアリル数をカウントしました。その結果をまとめたのが次の2つのデータです。以降はこれらデータを使用していきます。
...
<a name="section3_1"></a>
#### 1. データの読み込み
　データを読み込んでみましょう。今回もタブ区切りテキストファイルです。
...
<a name="section3_4"></a>
#### 4. 任意セルのデータ抽出
　任意のセルのデータを抽出してみましょう。「2. 任意の行のデータ抽出」と「3. 任意の列のデータ抽出」を組み合わせれば実現できます。ここでは行番号と列番号で任意セルの座標を指定してデータを取り出します。
...
　Pythonには、グラフ描画方法がいくつかあります。今回はPandasの描画機能を使いました。他には __Matplotlib__ と呼ばれるライブラリを使う方法もあります（Pandasの描画機能も内部ではMatplotlibが使われています）。Matplotlibの使い方は、多くのウェブサイトで紹介されているので、興味があれば調べてみてください。
